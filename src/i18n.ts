 ...
 * 加载单个语言文件
 */
export const getLocale = async (locale: string): Promise<AbstractIntlMessages> => {
...
export const getMessagesForLocale = async (locale: string): Promise<AbstractIntlMessages> => {
  ...
  // 非默认语言：先加载英文，再合并当前语言
  const defaultLocaleMessages = await getLocale(appConfig.i18n.defaultLocale);
  ...
  try {
    ...
    console.error('getMessagesForLocale deepmerge:', errorMessage);
    // 出错时回退到默认语言
